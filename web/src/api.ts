...
async function request(path:string,method:'GET'|'POST',body?:unknown,extraHeaders:Record<string,string>={}){
  const r=await fetch(BASE+path,{method,credentials:'include',headers:{'content-type':'application/json',...extraHeaders},body:body===undefined?undefined:JSON.stringify(body)});
...
export const api={
  ...
  authStatus:()=>request('/v1/auth/status','GET'),
  login:(token:string)=>request('/v1/auth/login','POST',{token}),
