...
const json=(res,status,body)=>{res.writeHead(status,{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':ALLOWED_ORIGIN,'Access-Control-Allow-Headers':'Content-Type, Authorization, X-Soly-Scopes, X-Soly-Confirmed, X-Soly-Max-Cost','Access-Control-Allow-Methods':'GET,POST,OPTIONS'});res.end(status===204?'':JSON.stringify(body));};
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(req.method==='GET'&&req.url==='/v1/oauth/status')return json(res,200,{ok:true,...await oauthStatus()});
    const oauthStart=req.url?.match(/^\/v1\/oauth\/(google|microsoft)\/start$/);if(req.method==='POST'&&oauthStart){const input=await body(req);return json(res,200,{ok:true,...await startOAuth(oauthStart[1],{returnTo:input.returnTo||''})});}
    ...
    if(req.method==='POST'&&req.url==='/v1/tools/execute'){const request=await body(req);const result=await executeToolRequest(request,{scopes:scopes(req),confirmed:confirmed(req),estimatedCostUsd:Number(request.estimatedCostUsd||0),maxCostUsd:maxCost(req),executors:adapters});return json(res,200,result);}
    if(req.method==='POST'&&req.url==='/v1/jobs'){const input=await body(req);const jobs=await readJobs();const job={jobId:input.jobId||`job-${Date.now()}`,state:'RECEIVED',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),...input};jobs.unshift(job);await writeJobs(jobs.slice(0,500));return json(res,201,job);}
    const jobMatch=req.url?.match(/^\/v1\/jobs\/([^/]+)$/); if(req.method==='GET'&&jobMatch){const jobs=await readJobs();const job=jobs.find(j=>j.jobId===decodeURIComponent(jobMatch[1]));return job?json(res,200,job):json(res,404,{error:'job_not_found'});}
    const approvalMatch=req.url?.match(/^\/v1\/jobs\/([^/]+)\/approve$/); if(req.method==='POST'&&approvalMatch){const patch=await body(req);const jobs=await readJobs();const idx=jobs.findIndex(j=>j.jobId===decodeURIComponent(approvalMatch[1]));if(idx<0)return json(res,404,{error:'job_not_found'});jobs[idx]={...jobs[idx],approvals:[...(jobs[idx].approvals||[]),{...patch,at:new Date().toISOString()}],updatedAt:new Date().toISOString()};await writeJobs(jobs);return json(res,200,jobs[idx]);}
